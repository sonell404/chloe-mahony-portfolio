...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  gallerySection.appendChild(createTitlePage(['CHLOE MAHONY', 'Portfolio'], page_number++));
  ...
  gallerySection.appendChild(createTitlePage(['chloemmahony@gmail.com', '0899769880'], page_number++, true));

  const projectPages = document.querySelectorAll(".project-page, .project-title-page, .title-page");
  ...
  const observer = new IntersectionObserver((entries) => {
    entries.forEach((entry) => {
      if (entry.isIntersecting) {
        ...
        pageNumberElement.textContent = pageNumber < 10 ? `0${pageNumber}` : pageNumber;
      }
    });
